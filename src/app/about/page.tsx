    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-md space-y-4">
          <div className="w-12 h-12 rounded-2xl bg-ukta-red/10 text-ukta-red flex items-center justify-center font-bold">
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-md space-y-4">
          <div className="w-12 h-12 rounded-2xl bg-ukta-gold/10 text-ukta-gold flex items-center justify-center font-bold">
      ...
      <div id="governing-docs" className="bg-slate-100 dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 space-y-6">
        ...
        <p className="text-xs text-slate-600 dark:text-slate-300">
          UKTA operates under strict non-profit governance standards in compliance with UK charity regulations. Download our constitution and policy documents below:
        </p>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            { title: 'UKTA Constitution (PDF)', size: '1.2 MB', ver: 'v2.1 (2024)' },
            ...
            <div key={idx} className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 flex flex-col justify-between space-y-3">
              <div>
                <span className="text-[10px] font-bold text-ukta-gold uppercase">{doc.ver}</span>
                <h3 className="text-xs font-bold text-slate-900 dark:text-white mt-1">{doc.title}</h3>
              </div>
              <button
                onClick={() => alert(`Downloading ${doc.title}...`)}
                className="w-full bg-slate-900 hover:bg-ukta-red text-white py-2 rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-1.5"
              >
