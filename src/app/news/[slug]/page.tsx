...
export default function NewsDetailPage() {
  ...
  if (!post) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center space-y-4">
        ...
        <Link href="/news" className="text-mitra-red underline text-xs font-bold">Back to News Hub</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8">
      <Link href="/news" className="inline-flex items-center gap-1 text-xs font-bold text-mitra-red dark:text-mitra-gold hover:underline">
        <ArrowLeft className="w-4 h-4" />
        <span>Back to All News</span>
      </Link>

      <div className="space-y-4">
        <span className="bg-mitra-red text-white text-xs font-bold px-3 py-1 rounded-full uppercase">
          {post.category}
        </span>
