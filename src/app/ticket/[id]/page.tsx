...
const DEMO_PAYMENTS: Record<string, {
  ...
  'pay-102': { id: 'pay-102', amount: 15.0, currency: 'GBP', status: 'Completed', customerName: 'Anil Varma', customerEmail: 'anil.v@example.co.uk', description: 'UKTA Ugadi Cultural Fest Entry Pass (1 Ticket)', paymentMethod: 'Stripe ApplePay', createdAt: new Date(Date.now() - 3600000 * 24).toISOString() },
  ...
  'pay-201': { id: 'pay-201', amount: 51.0, currency: 'GBP', status: 'Completed', customerName: 'Mahesh Babu G', customerEmail: 'member@ukta.org.uk', description: 'Donation — Ganesh Mahotsav 2026 Seva Fund', paymentMethod: 'Stripe Card', createdAt: new Date(Date.now() - 3600000 * 10).toISOString() },
  'pay-202': { id: 'pay-202', amount: 25.0, currency: 'GBP', status: 'Completed', customerName: 'Mahesh Babu G', customerEmail: 'member@ukta.org.uk', description: 'Pooja Booking — Ganesh Chaturthi Morning Slot', paymentMethod: 'Stripe ApplePay', createdAt: new Date(Date.now() - 3600000 * 72).toISOString() },
};
...
export async function generateMetadata({ params }: PageProps) {
  return {
    ...
    description: 'Official donation receipt and ticket from MITRA UK (UKTA). Verify this contribution record.',
  };
}
...
export default async function TicketPage({ params }: PageProps) {
  ...
  const ticketUrl = `${process.env.NEXT_PUBLIC_BASE_URL || 'https://ukta.org.uk'}/ticket/${canonicalToken}`;
