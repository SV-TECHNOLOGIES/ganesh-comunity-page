...
export default function Header({ previewMode = false }: HeaderProps = {}) {
  ...
  return (
    <header className={`${previewMode ? 'relative w-full' : 'sticky top-0 z-50'} bg-[#FFF8F0]/96 backdrop-blur-md border-b border-[#E65C00]/20 shadow-[0_2px_20px_rgba(61,26,0,0.06)] transition-all`}>
          ...
          <div className="max-w-7xl mx-auto flex justify-between items-center text-[11px]">
            <div className="flex items-center gap-2">
              ...
              <span className="hidden md:inline text-white/90">
                13th – 19th Sept 2026 · Slough &amp; Langley College, Langley Road, SL3 8GW
              </span>
            </div>
            <div className="flex items-center space-x-3.5">
              ...
              <span className="text-white/40">|</span>
              <Link href="/admin" className="text-white/80 hover:text-white flex items-center gap-1 transition-colors text-[11px] font-semibold">
          ...
          <nav className="hidden lg:flex items-center space-x-1 xl:space-x-3 text-xs font-bold uppercase tracking-wider whitespace-nowrap">
            ...
            {NAV_CONFIG.SHOW_ABOUT_DROPDOWN ? (
              ...
              <>
                <Link 
                  href="/about" 
                  className={`px-3 py-2 rounded-lg transition-colors hover:text-[#E65C00] hover:bg-[#FFF0E0] ${isCurrent('/about') ? 'text-[#E65C00] font-black bg-[#FFF0E0]' : 'text-[#3D1A00]'}`}
                >
                  About Us
                </Link>
                {NAV_CONFIG.SHOW_LEADERSHIP && (
                  <Link 
                    href="/leadership" 
                    className={`px-3 py-2 rounded-lg transition-colors hover:text-[#E65C00] hover:bg-[#FFF0E0] ${isCurrent('/leadership') ? 'text-[#E65C00] font-black bg-[#FFF0E0]' : 'text-[#3D1A00]'}`}
                  >
                    Leadership
                  </Link>
                )}
            ...
            {/* Community Dropdown (Groups Sponsors, Telugu Business, Media, Membership) */}
            <div 
              className="relative"
              ...
              {communityDropdown && (
                <div className="absolute top-full left-0 w-64 bg-[#FFFAF5] shadow-2xl rounded-2xl p-2 border border-[#E65C00]/20 z-50 animate-in fade-in-50 zoom-in-95 duration-150">
                  {NAV_CONFIG.SHOW_MEMBERSHIP && (
                    <Link href="/membership" className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-xs text-[#3D1A00] hover:bg-[#FFF0E0] hover:text-[#E65C00] transition-colors">
                      <Users className="w-4 h-4 text-[#E65C00]" />
                      <div>
                        <div className="font-bold">Membership</div>
                        <div className="text-[10px] text-[#6B3A2A] normal-case">Become a Mitra UK member</div>
                      </div>
                    </Link>
                  )}
